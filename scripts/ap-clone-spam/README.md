...
The AutoMarauder AP Clone Spam program is developed in Typescript and built for the mJS JavaScript engine on the Flipper Zero that provides WiFi access point beacon spam attacks via the [Marauder firmware](https://github.com/justcallmekoko/ESP32Marauder) on a Flipper Zero WiFi dev board.

AutoMarauder AP Clone Spam lets you scan nearby WiFi access points, browse discovered SSIDs, and perform a beacon spam attack by cloning a selected access point with spaces between the characters to look like valid access points, all from the Flipper Zero UI. without the need to manually interact with Marauder directly.
...
AutoMarauder AP Clone Spam depends on the **AutoMarauder Shared Library**, which provides the runtime modules loaded at startup:
...
These libraries **must be installed to the Flipper Zero separately** before running AutoMarauder AP Clone Spam. The application loads them at runtime using mJS `load()` calls and will crash on startup if any of them are missing.
...
To compile AutoMarauder AP Clone Spam, the TypeScript type definitions for the shared library must be present locally. Copy them from the built library output into this project's `lib/` directory:
...
- **Beacon Spam** — Perform a beacon spam attack by cloning a selected access point
