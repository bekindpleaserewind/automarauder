...
View.addView("scan_ap_clone_spam_enable", "scan_ap_options", dialog.makeWith({
    header: "Clone AP and Spam",
    text: "",
    left: "Enable",
    right: "Disable",
    center: "Back",
...
View.addView("scan_ap_clone_spam_success", "scan_ap_clone_spam_enable", dialog.makeWith({
    text: "Clone AP and Spam Success!",
    center: "Back",
}), null);

View.addView("scan_ap_clone_spam_status_disabled", "scan_ap_clone_spam_enable", dialog.makeWith({
    header: "Successful!",
    text: "Stopped Clone and Spam",
    center: "Back",
}), null);

View.subscribe("scan_ap_clone_spam_enable", "input", function(sub, button: string) {
    if(button === "right") {
        Wifi.stopScanAps();
        View.show("scan_ap_clone_spam_status_disabled");
    } else if(button === "center") {
        View.show("scan_ap_options");
    } else if(button === "left") {
        Wifi.startCloneApSpam();
        View.show("scan_ap_clone_spam_success")
    }
});
...
View.subscribe("scan_ap_options", "chosen", function(sub, index: number) {
    if(index === 0) {
        View.getView("scan_ap_clone_spam_enable").set("text", App.selectedSsid)
        View.show("scan_ap_clone_spam_enable");
    }
});
...
View.subscribe("scan_ap_clone_spam_status_disabled", "input", function(sub, button: string) {
    if(button === "center") {
        View.show('scan_ap_clone_spam_enable');
    }
});

View.subscribe("scan_ap_clone_spam_success", "input", function(sub, button) {
    if(button === "center") {
        View.show("scan_ap_clone_spam_enable");
    }
});
