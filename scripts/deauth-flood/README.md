...
The AutoMarauder Deauth Flood program is developed in Typescript and built for the mJS JavaScript engine on the Flipper Zero that provides WiFi access point deauthentication flood attacks via the [Marauder firmware](https://github.com/justcallmekoko/ESP32Marauder) on a Flipper Zero WiFi dev board.

AutoMarauder Deauth Flood lets you scan nearby WiFi access points, browse discovered SSIDs, and perform a deauthentication flood attack against a selected access point, all from the Flipper Zero UI. without the need to manually interact with Marauder directly.
...
AutoMarauder Deauth Flood depends on the **AutoMarauder Shared Library**, which provides the runtime modules loaded at startup:
...
These libraries **must be installed to the Flipper Zero separately** before running AutoMarauder Deauth Flood. The application loads them at runtime using mJS `load()` calls and will crash on startup if any of them are missing.
...
To compile AutoMarauder Deauth Flood, the TypeScript type definitions for the shared library must be present locally. Copy them from the built library output into this project's `lib/` directory:
...
- **Deauth Flood** — Perform a deauthentication flood attack against a selected access point
