...
View.subscribe("scan_ap_options", "chosen", function(sub, index: number) {
    if(index === 0) {
        if(View.hasView("scan_ap_information")) {
            ...
            View.getView("scan_ap_information_essid").set("text", App.info.essid);
            View.getView("scan_ap_information_bssid").set("text", App.info.bssid);
            View.getView("scan_ap_information_rssi").set("text", App.info.rssi);
            View.getView("scan_ap_information_security").set("text", App.info.security);
            View.getView("scan_ap_information_selected").set("text", App.info.selected);
            View.getView("scan_ap_information_frames").set("text", App.info.frames);
            View.getView("scan_ap_information_stations").set("text", App.info.stations);
            View.getView("scan_ap_information_eapol").set("text", App.info.eapol);

            View.getView("scan_ap_information").setChildren([
                "   ESSID: " + App.info.essid,
                "   BSSID: " + App.info.bssid,
                "    RSSI: " + App.info.rssi,
                "Security: " + App.info.security,
                "Selected: " + App.info.selected,
                "  Frames: " + App.info.frames,
                "Stations: " + App.info.stations,
                "   EAPOL: " + App.info.eapol,
            ]);
...
View.subscribe("scan_ap_information_selected", "input", function(sub, button) { View.show("scan_ap_information"); });
View.subscribe("scan_ap_information_frames", "input", function(sub, button) { View.show("scan_ap_information"); });
View.subscribe("scan_ap_information_stations", "input", function(sub, button) { View.show("scan_ap_information"); });
View.subscribe("scan_ap_information_eapol", "input", function(sub, button) { View.show("scan_ap_information"); });
...
View.subscribe("scan_ap_information", "chosen", function(sub, index, eventLoop) {
    if(index === 0) {
        View.show("scan_ap_information_essid");
    } else if(index === 1) {
        View.show("scan_ap_information_bssid");
    } else if(index === 2) {
        View.show("scan_ap_information_rssi");
    } else if(index === 3) {
        View.show("scan_ap_information_security");
    } else if(index === 4) {
        View.show("scan_ap_information_selected");
    } else if(index === 5) {
        View.show("scan_ap_information_frames");
    } else if(index === 6) {
        View.show("scan_ap_information_stations");
    } else if(index === 7) {
        View.show("scan_ap_information_eapol");
    }
});
