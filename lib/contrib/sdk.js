...
async function build(config, extra) {
    await esbuild.build({
...
async function upload(config, extra) {
    //const appFile = fs.readFileSync(config.input, "utf8");
    const appFile = fs.readFileSync(extra.src, "utf8");
    ...
    await waitFor(">: ", 1000);
    port.write(`storage remove ${extra.dst}\x0d`);
    port.drain();
    await waitFor(">: ", 1000);

    //port.write(`storage write_chunk ${config.output} ${appFile.length}\x0d`);
    console.log(`storage write_chunk ${extra.dst} ${appFile.length}`);
    port.write(`storage write_chunk ${extra.dst} ${appFile.length}\x0d`);
    port.drain();
    await waitFor("Ready", 1000);
    console.log(`writing file ${extra.dst}`);
    port.write(appFile);
    port.drain();
    await waitFor(">: ", 3000);
    process.exit(0);
...
async function _mkdir(config, extra) {
    const serialPorts = await SerialPort.list();
    ...
    let dirs = extra.dir.split('/');
    let path = '';
    for(let i = 1; i < dirs.length; i++) {
        path = `${path}/${dirs[i]}`;
        console.log(`storage mkdir ${path}`);
        port.write(`storage mkdir ${path}\x0d`);
        port.drain();
        await waitFor(">: ", 1000);
    }
...
async function remove(config, extra) {
    const serialPorts = await SerialPort.list();
...
(async () => {
    ...
    if(process.argv.length > 2) {
        dstDir = process.argv[3];
        if(process.argv.length > 3) {
            srcFile = process.argv[4];
            if(process.argv.length > 4) {
                dstFileName = process.argv[5];
    ...
    await commands[command](config[command], extra);
})();
