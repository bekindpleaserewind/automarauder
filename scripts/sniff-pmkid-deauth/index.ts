...
View.addView("sniff_pmkid_running", "sniff_pmkid_confirm", dialog.makeWith({
    text: "Sniffing PMKID",
    center: "Back",
}), null)

View.addView("sniff_pmkid_disabled", "sniff_pmkid_running", dialog.makeWith({
    text: "Disabled PMKID Sniffing",
    center: "Back",
}), null)
...
View.subscribe("sniff_pmkid_confirm", "input", function(sub, button) {
    if(button === "left") {
        ...
        Wifi.stopSniffPMKID();
        View.show("sniff_pmkid_disabled");
    } else {
        View.show('scan_ap_options');
    }
});

View.subscribe("sniff_pmkid_running", "input", function(sub, button) {
    if(button === "center") {
        View.show("sniff_pmkid_confirm");
    }
});

View.subscribe("sniff_pmkid_disabled", "input", function(sub, button) {
    if(button === "center") {
        View.show("sniff_pmkid_confirm");
    }
});
