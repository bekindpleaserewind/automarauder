...
View.addView("scan_ap_deauth_flood_enable", "scan_ap_options", dialog.makeWith({
    header: "Deauth Flood",
    text: "",
    left: "Enable",
    right: "Disable",
...
View.addView("scan_ap_deauth_flood_success", "scan_ap_deauth_flood_enable", dialog.makeWith({
    text: "Deauth Flood Success!",
    center: "Back",
}), null);

View.addView("scan_ap_deauth_flood_status_disabled", "scan_ap_deauth_flood_enable", dialog.makeWith({
    header: "Success",
    text: "Stopped Deauth Flood",
    center: "Back",
}), null);

View.subscribe("scan_ap_deauth_flood_enable", "input", function(sub, button: string) {
    if(button === "right") {
        Wifi.stopScanAps();
        View.show("scan_ap_deauth_flood_status_disabled");
    } else if(button === "center") {
        View.show("scan_ap_options");
    } else if(button === "left") {
        Wifi.deauth();
        View.show("scan_ap_deauth_flood_success")
    }
});
...
View.subscribe("scan_ap_options", "chosen", function(sub, index: number) {
    if(index === 0) {
        View.getView("scan_ap_deauth_flood_enable").set("text", App.selectedSsid)
        View.show("scan_ap_deauth_flood_enable");
    }
});
...
View.subscribe("scan_ap_deauth_flood_status_disabled", "input", function(sub, button: string) {
    if(button === "center") {
        View.show('scan_ap_deauth_flood_enable');
    }
});

View.subscribe("scan_ap_deauth_flood_success", "input", function(sub, button) {
    if(button === "center") {
        View.show("scan_ap_deauth_flood_enable");
    }
});
