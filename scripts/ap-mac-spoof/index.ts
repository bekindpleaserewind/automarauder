...
View.subscribe("scan_ap_mac_spoofing_enable", "input", function(sub, button: string) {
    if(button === "right") {
        View.show("loading");
        Wifi.reboot();
...
View.subscribe("scan_ap_options", "chosen", function(sub, index: number) {
    if(index === 0) {
        View.getView("scan_ap_mac_spoofing_enable").set("text", App.selectedSsid)
...
View.subscribe("scan_ap_mac_spoofing_status_disabled", "input", function(sub, button: string) {
    if(button === "center") {
        View.show('scan_ap_confirm');
    }
});

View.subscribe("scan_ap_mac_spoof_success", "input", function(sub, button) {
    if(button === "center") {
        View.show("scan_ap_mac_spoofing_enable");
    }
});
